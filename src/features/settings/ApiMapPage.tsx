...
import type { QueryRequest, UModelElement } from '../../api/types'
...
type ApiCallResponse = Pick<ApiResult, 'status' | 'statusText' | 'ok' | 'payload'>
...
export function ApiMapPage({ api, workspaceId }: { api: UModelApiClient; workspaceId: string }) {
  const { t } = useI18n()
  ...
  async function run() {
    ...
    try {
      ...
      const response = api.baseUrl.startsWith('mock://')
        ? await runMockApiCall(api, selected, workspaceId, pathParams, queryParams, payload)
        : await fetchApiCall(api, selected, requestUrl, payload)
      setResult({
...
async function runMockApiCall(
  ...
  const workspace = pathParams.workspace || workspaceId
  const sample = pathParams.sample || 'multi-domain-quickstart'
  const body = payloadRecord(payload)
  let responsePayload: unknown

  switch (selected.id) {
    case 'health':
      responsePayload = await api.health()
      break
    case 'workspace-list':
      responsePayload = await api.listWorkspaces({
        includeDeleted: queryParams.include_deleted === 'true',
        includeConflicts: queryParams.include_conflicts === 'true',
      })
      break
    case 'workspace-create':
      responsePayload = await api.createWorkspace({
        id: stringValue(body.id) || 'demo-api',
        name: stringValue(body.name) || undefined,
        description: stringValue(body.description) || undefined,
        labels: stringRecord(body.labels),
        config: groupedRecord(body.config),
      })
      break
    case 'workspace-get':
      responsePayload = await api.getWorkspace(workspace)
      break
    case 'workspace-update':
      responsePayload = await api.updateWorkspace(workspace, {
        name: stringValue(body.name) || undefined,
        description: stringValue(body.description) || undefined,
        labels: stringRecord(body.labels),
        config: groupedRecord(body.config),
        if_match_version: numberValue(body.if_match_version),
        replace_labels: booleanValue(body.replace_labels),
        replace_config: booleanValue(body.replace_config),
      })
      break
    case 'workspace-delete':
      responsePayload = await api.deleteWorkspace(workspace)
      break
    case 'query-execute':
      responsePayload = await api.query(workspace, {
        query: stringValue(body.query) || '.umodel | limit 1000',
        limit: numberValue(body.limit),
        timeout_ms: numberValue(body.timeout_ms),
        time_range: timeRangeValue(body.time_range),
        parameters: payloadRecord(body.parameters),
      })
      break
    case 'query-explain':
      responsePayload = await api.explain(workspace, {
        query: stringValue(body.query) || '.topo | limit 1000',
        limit: numberValue(body.limit),
        timeout_ms: numberValue(body.timeout_ms),
        time_range: timeRangeValue(body.time_range),
        parameters: payloadRecord(body.parameters),
      ...
      break
    case 'umodel-import':
      responsePayload = await api.importUModel(workspace, {
        path: stringValue(body.path) || 'examples/umodel',
        common_schema_packs: stringArray(body.common_schema_packs),
      })
      break
    case 'umodel-validate':
      responsePayload = await api.validateUModel(workspace, umodelElements(body.elements))
      break
    case 'umodel-write':
      responsePayload = await api.putUModel(workspace, umodelElements(body.elements))
      break
    case 'umodel-delete':
      responsePayload = await api.deleteUModel(workspace, stringArray(body.ids))
      break
    case 'sample-import':
      responsePayload = await api.importSampleData(workspace, sample)
      break
    case 'entities-write':
      responsePayload = await api.writeEntities(workspace, {
        entities: recordArray(body.entities),
        idempotency_key: stringValue(body.idempotency_key) || undefined,
        partial_success: booleanValue(body.partial_success),
      })
      break
    case 'entities-expire':
      responsePayload = await api.expireEntities(workspace, {
        ids: stringArray(body.ids),
        reason: stringValue(body.reason) || undefined,
      })
      break
    case 'relations-write':
      responsePayload = await api.writeRelations(workspace, {
        relations: recordArray(body.relations),
        idempotency_key: stringValue(body.idempotency_key) || undefined,
        partial_success: booleanValue(body.partial_success),
      })
      break
    case 'relations-expire':
      responsePayload = await api.expireRelations(workspace, {
        ids: stringArray(body.ids),
        reason: stringValue(body.reason) || undefined,
      })
      break
    case 'agent-discover':
      responsePayload = await api.discoverAgent(workspace)
      break
    case 'agent-resource':
      responsePayload = await api.readAgentResource(workspace, stringValue(body.uri) || `umodel://workspace/${workspace}/overview`)
      break
    case 'agent-tool':
      responsePayload = await api.executeAgentTool(
        workspace,
        stringValue(body.name) || 'query.execute',
        payloadRecord(body.arguments),
      )
      break
    default:
      responsePayload = { ok: false, message: `Unsupported mock API: ${selected.id}` }
  }

  return { status: 200, statusText: 'OK', ok: true, payload: responsePayload }
}
...
function umodelElements(value: unknown): UModelElement[] {
  return recordArray(value).map((item) => ({
    kind: stringValue(item.kind),
    domain: stringValue(item.domain),
    name: stringValue(item.name),
    version: stringValue(item.version) || undefined,
    spec: payloadRecord(item.spec),
  }))
}

function timeRangeValue(value: unknown): QueryRequest['time_range'] {
  const record = payloadRecord(value)
  return {
    from: stringValue(record.from) || undefined,
    to: stringValue(record.to) || undefined,
  }
}
...
function stringRecord(value: unknown): Record<string, string> | undefined {
  const record = payloadRecord(value)
  const entries = Object.entries(record).filter(([, item]) => typeof item === 'string') as Array<[string, string]>
  return entries.length > 0 ? Object.fromEntries(entries) : undefined
}

function groupedRecord(value: unknown): Record<string, Record<string, unknown>> | undefined {
  const record = payloadRecord(value)
  const entries = Object.entries(record).filter(([, item]) => item && typeof item === 'object' && !Array.isArray(item)) as Array<[string, Record<string, unknown>]>
  return entries.length > 0 ? Object.fromEntries(entries) : undefined
}
